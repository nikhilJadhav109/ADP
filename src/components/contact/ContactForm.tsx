...
const ContactForm: React.FC<ContactFormProps> = () => {
  ...
  const handleWhatsAppSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // No need to clear form data here if you want to keep it for Google Form submit
    } catch (error) {
  ...
  const handleGoogleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Form data sent to Google Form successfully!");

      setFormData({
        name: "",
        email: "", // If you keep an email field, ensure it's cleared.
        phone: "",
        projectType: "",
        size: "",
        budget: "",
        timeline: "",
        message: "", // Clears the project location or general message.
      });
      setIsSubmitted(true); // You might want a different state or message for Google Form success
    } catch (error) {
      console.error("Error submitting to Google Form:", error);
      setGoogleFormStatus("Failed to submit to Google Form. Please try again.");
      alert("Failed to send data to Google Form. Check console for details.");
    } finally {
          ...
          {formData.projectType === "residential" && (
            <motion.div>
              ...
              <select
                ...
                value={formData.size}
                onChange={handleChange}
                required={formData.projectType === "residential"} // Make it required only if residential is selected
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all duration-200 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 shadow-sm"
              >
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            >
              ...
              >
                ...
                <option value="Long Term (12+ months)">Long term (1+ years)</option>
              </select>
            </motion.div>
          </div>
          <motion.div
            ...
            >
              {isSubmittingGoogleForm ? (
                ...
                <>
                  ...
                  {/* Added className="mr-2" for Tailwind margin */}
                  Send
                </>
              )}
            </motion.button>
